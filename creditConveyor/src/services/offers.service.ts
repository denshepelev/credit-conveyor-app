...
export class OfferService {
   ...
   * get offer list
   */
  public getLoanOfferDTOList(parameters: LoanApplicationRequestDTO): Array<LoanOfferDTO> {
    const loanOfferDTOList: Array<LoanOfferDTO> = [];

    const loanApplicationRequestDTO: LoanApplicationRequestDTO = new LoanApplicationRequestDTO();
    ...
    const errors: Array<ValidationError> = validateSync(loanApplicationRequestDTO, {
    ...
    const addOffer = (
      ...
      const totalAmount: number = calculator.calculateTotalAmount(amount, term, isInsuranceEnabled);

      const totalRate = calculator.calculateTotalRate(this.BASE_RATE, isInsuranceEnabled, isSalaryClient);

      const rateProportion = calculator.calculateRateProportion(totalRate);

      const monthlyPayment = calculator.calculateMonthlyPayment(totalAmount, term, rateProportion);
    ...
    addOffer(1, loanApplicationRequestDTO.amount, loanApplicationRequestDTO.term, false, false);

    //case_2: Insurance = true✅, SalaryClient = false❌;
    addOffer(2, loanApplicationRequestDTO.amount, loanApplicationRequestDTO.term, true, false);

    //case_3: Insurance = false❌, SalaryClient = true✅;
    addOffer(3, loanApplicationRequestDTO.amount, loanApplicationRequestDTO.term, false, true);

    //case_4: Insurance = true✅, SalaryClient = true✅;
    addOffer(4, loanApplicationRequestDTO.amount, loanApplicationRequestDTO.term, true, true);
